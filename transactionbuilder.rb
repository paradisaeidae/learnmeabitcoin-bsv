...
class Transaction
 ...
 def sign(i, privatekey, placeholder)
  ...
  while signature.nil?
   ...
   signature = ECDSA.sign(group, privatekey, message, temp_key) end
...
=end
  ...
  inputs[i].scriptSig = scriptSig end
 ...
 def serialize()
  ...
  if (@inputs.count > 0)
   ...
   @inputs.each { |input| serialized += input.serialize() } end # inputs
  
  if (@outputs.count > 0)
   serialized += varint(@outputs.count)                            # output count
   @outputs.each { |output| serialized += output.serialize() } end # outputs
