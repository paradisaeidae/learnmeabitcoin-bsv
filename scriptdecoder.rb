...
class Script
  ...
  def initialize(data)
   if data =~ /^\S*$/ then @hex, @asm = data, hex_to_asm(data)                 # hex (no whitespaces, so probably hex script)
   else @hex, @asm = asm_to_hex(data), data end                                # asm
   @type = get_type(@asm) end
  ...
  def hex_to_asm(hex)
   ...
   while byte = bytes.shift
    ...
    if int > 0 && int < 0x4b
     ...
     asm << data
    else asm << OPCODES.get_opcode(byte) end end # Get OPCODE from hex value (checks list of constants)
   return asm.join(' ') end
  ...
  def asm_to_hex(asm)
   ...
   while piece = pieces.shift
    ...
    hex << OPCODES.const_get(piece) # Get hex value for OPCODE
     ...
     hex << push + piece end end # Just add the bytes if it is not an OPCODE constant
   return hex end

  def get_type(asm)
   asm = asm.split
   if asm.length == 2 && asm[1] == 'OP_CHECKSIG' then :p2pk
   elsif asm.length == 5 && asm[0] == 'OP_DUP' && asm[1] == 'OP_HASH160' && asm[3] == 'OP_EQUALVERIFY' && asm[4] == 'OP_CHECKSIG' then :p2pkh
   # elsif asm.length == 3 && asm[0] == 'OP_HASH160' && asm[2] == 'OP_EQUAL' then :p2sh (NOT REAL BITCOIN)
   elsif asm[0] =~ /OP_[0-9]*/ && asm[-2] =~ /OP_[0-9]*/ && asm[-1] == 'OP_CHECKMULTISIG' then :p2ms
   elsif asm.length == 2 && asm[0] == 'OP_RETURN' then :return
  ...
  def Script.run(*scripts) # OP_CHECKSIG needs the hash of the thing to sign for it to work
    # 0. If locking script given and it's P2SH, run specific run_p2sh validation for it DEPRECATE
    # return Script.run_p2sh(scripts) if scripts.size > 1 && scripts[1].type == :p2sh DEPRECATE
    raise 'p2sh is not part of BSV (Bitcoin)' if scripts.size > 1 && scripts[1].type == :p2sh
    ...
    while opcode = script.shift                    # Take each element of the script array
     ...
     else stack << opcode end                            # If not opcode, just add data to the stack (anything that is not an OP_CODE method)
     yield(script, stack) if block_given? end# Display (Can give a block to this method to display stuff.)
...
=end
  ...
  def self.get_opcode(hex) self.constants.select {|constant| hex == self.const_get(constant) } end
  ...
  def self.OP_RETURN(stack) raise "Script is invalid. (OP_RETURN always invalidates a script.)" end end end
...
if yn == '' || 'y'
 ...
 stack = Script.run(unlockingscript, lockingscript) { |script, stack| #Script.run gives the script and stack to this block of code
  ...
  puts stack.reverse # Want to see the top stack item at the top
  gets }
 if Script.validate(stack) then
 else puts "This is not a valid script." end end
